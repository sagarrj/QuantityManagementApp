...
/*
Also we would like to catch people who are driviing at unsafe speed on the highway
To help us do tha twe would like to identify journeys where driver does either of the following
...
public class Solution {
    ...
    static class LogFile {
        ...
        public int countJourneys() {
            ...
            for (LogEntry entry : logEntries) {
                ...
                String plate = entry.licensePlate;    // Car TS001 ------- enter into the map Key TS001 - 0
						      // car TS002 ------- enter into the map Key TS002 -0
                ...
                if (entry.boothType.equals("ENTRY")) {
                    ...
                    if (activeJourneys.containsKey(plate) // Car TS001 --------------check if the TS001 inside the map make this 0 -> 1
                            && activeJourneys.get(plate)
                            .equals(entry.direction)) {
        ...
        // --------------------------------------------------
        // Catch Unsafe Drivers
        // --------------------------------------------------
        public int countUnsafeJourneys() {
            ...
            for (LogEntry entry : logEntries) {
                ...
                else if (entry.boothType.equals("MAINROAD")
                        || entry.boothType.equals("EXIT")) {

                    if (activeEntry.containsKey(plate)) {

                        LogEntry previous =
                                lastCheckpoint.get(plate);

                        double time =
                                entry.timestamp - previous.timestamp;

                        int distance =
                                Math.abs(entry.location -
                                        previous.location);

                        if (distance > 0 && time > 0) {

                            double speed =
                                    (distance * 3600.0) / time;
    ...
    public static void testCatchSpeeder() {
